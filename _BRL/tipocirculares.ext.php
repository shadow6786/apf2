<?php
require_once("tipocirculares.class.php");

class ctipocirculares_ext extends ctipocirculares
{
    ...
    public function get_tipo_circulares()
    {
       try
	   {

		$sql = "SELECT * 
		FROM  `cnf_tipocirculares` 
		WHERE  `activo_tcc` =  'S'";
   		$rs = mysql_query($sql);
		$total_filas = mysql_num_rows($rs);
    ...
    public function get_tipo_circular($var)
    {
       try
	   {

		$sql = "SELECT *, IFNULL(rol_tcc,0) as requiere_login
		FROM cnf_tipocirculares where id_tcc = ".$var." and activo_tcc = 'S'";
   		$rs = mysql_query($sql);
		$total_filas = mysql_num_rows($rs);
