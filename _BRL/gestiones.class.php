...
class cgestiones
{
	...
	public function borrar()
	{
  		 try
		 {
			$sql = "DELETE FROM cnf_gestiones WHERE id_gst=" . mysql_real_escape_string($this->id_gst);
  			mysql_query($sql);
			if (mysql_errno()) 
			{ 
	...
	public function insertar()	
	{
		try
		 {
			$sql = "INSERT INTO cnf_gestiones SET
					
					nombre_gst='" . mysql_real_escape_string($this->nombre_gst) . "',
  
					activo_gst='" . mysql_real_escape_string($this->activo_gst) . "',
	...
	public function actualizar()
	{
		try
		{
  			$sql = "UPDATE cnf_gestiones SET
				...
				usuario_mod=" . mysql_real_escape_string($this->usuario_mod) . "
      			WHERE
        		id_gst="   . mysql_real_escape_string($this->id_gst);
	...
	public function ObtenerUnRegistro()
  	{
    	try
		{
  			$sql = 'SELECT * FROM cnf_gestiones WHERE id_gst=\'' . mysql_real_escape_string($this->id_gst)."'";
			$result = mysql_query($sql);	
			$c = mysql_num_rows($result);
			if( $c > 0)
			{
				$row = mysql_fetch_array($result);			   		
				$this->id_gst = $row['id_gst'];
				
				$this->nombre_gst = $row['nombre_gst'];
				$this->activo_gst = $row['activo_gst'];
