...
class ctipocirculares
{
	...
	public function borrar()
	{
  		 try
		 {
			$sql = "DELETE FROM cnf_tipocirculares WHERE id_tcc=" . mysql_real_escape_string($this->id_tcc);
  			mysql_query($sql);
			if (mysql_errno()) 
			{ 
	...
	public function insertar()	
	{
		try
		 {
			$sql = "INSERT INTO cnf_tipocirculares SET
					
					nombre_tcc='" . mysql_real_escape_string($this->nombre_tcc) . "',
  
					activo_tcc='" . mysql_real_escape_string($this->activo_tcc) . "',
  
					rol_tcc='" . mysql_real_escape_string($this->rol_tcc) . "',
	...
	public function actualizar()
	{
		try
		{
  			$sql = "UPDATE cnf_tipocirculares SET
				...
				usuario_mod=" . mysql_real_escape_string($this->usuario_mod) . "
      			WHERE
        		id_tcc="   . mysql_real_escape_string($this->id_tcc);
	...
	public function ObtenerUnRegistro()
  	{
    	try
		{
  			$sql = 'SELECT * FROM cnf_tipocirculares WHERE id_tcc=\'' . mysql_real_escape_string($this->id_tcc)."'";
			$result = mysql_query($sql);	
			$c = mysql_num_rows($result);
			if( $c > 0)
			{
				$row = mysql_fetch_array($result);			   		
				$this->id_tcc = $row['id_tcc'];
				
				$this->nombre_tcc = $row['nombre_tcc'];
				$this->activo_tcc = $row['activo_tcc'];
				$this->rol_tcc = $row['rol_tcc'];
	...
	public function BorrarPorFiltro()
	{
	  	 ...
	  	 try
		 {
			$sql = "DELETE FROM cnf_tipocirculares ";
    ...
    public function buscar($pag,$todos = 'N')
    {
       try
	   {
		    $sql = "SELECT * FROM cnf_tipocirculares";
        	$sql_total = "SELECT count(*) as cantidad_filas FROM cnf_tipocirculares"; 
			$sql_filtrado = "";
			$where = array();
