...
<head>
	...
	function abrircaja()
	{
		...
		else
		{
			...
			{
				alert('Debe ingresar un valor num�rico en el campo monto');
				monto.focus();
			}
			else
			{
		...
		else
		{
			$.ajax({
				...
				success: function(html){
						if (html == "1")
				 	{
						document.getElementById("msg_mensaje_cambioclave").innerHTML = '<div class="ui-widget"><div class="ui-state-highlight ui-corner-all" > <p><span class="ui-icon ui-icon-info" style="float: left; margin-right: .3em; margin-left: .3em;"></span>Clave cambiada con �xito</p></div></div>';
						actualizarTablaTurnoAbierto();
					}
					else
					{
						document.getElementById("msg_mensaje_cambioclave").innerHTML = '<div class="ui-widget"><div class="ui-state-error ui-corner-all"> <p><span class="ui-icon ui-icon-alert" style="float: left; margin-right: .3em; margin-left: .3em;"></span>Ocurri� un error cambiando la clave: '+html+'</p></div></div>';}
        ...
        <div class="cajas" style="width:250px">
         <div class="ui-tabs ui-widget ui-widget-content ui-corner-all ">
            ...
            <div id="msg_mensaje_cambioclave"></div>
             La �ltima vez que cambi� su clave fue hace <strong><?php
			$objusuario=new cusuarios_ext;
			$objusuario->id_usr = $seguridad->userid;
			$objusuario->ObtenerUnRegistro();
			$fecha_ultimo_cambio = $objusuario->fechahora_mod;
         ...
         </strong> d&iacute;as. Es recomendable cambiarla por lo menos una vez al mes.<br><br>
